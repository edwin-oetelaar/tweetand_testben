...
int8_t socket(uint8_t sn, uint8_t protocol, uint16_t port, uint8_t flag)
{
    ...
    while (getSn_CR(sn)); /* can hang TODO */
    sock_io_mode |= ((flag & SF_IO_NONBLOCK) << sn);
    sock_is_sending &= ~(1 << sn);
    sock_remained_size[sn] = 0;
    sock_pack_info[sn] = 0;
    while (getSn_SR(sn) == SOCK_CLOSED); /* can hang TODO */
    return (int8_t) sn;
}
...
#endif
    default:
        wiz_recv_ignore(sn, pack_len);	// read data discarding it
...
#endif
    case SO_SENDBUF:
        *(uint16_t *) arg = getSn_TX_FSR(sn);	/* size of free TX buffer */
    case SO_RECVBUF:
        *(uint16_t *) arg = getSn_RX_RSR(sn);	/* size of data in RX buffer */
    case SO_STATUS:
        *(uint8_t *) arg = getSn_SR(sn);	/* socket status : */
        break;
    case SO_REMAINSIZE:
